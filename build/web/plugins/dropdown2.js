 ...
 *<br />Project page <a href="https://github.com/adobe-accessibility/Accessible-Menu">https://github.com/adobe-accessibility/Accessible-Menu</a>
 * @version 0.1
...
(function ($, window, document) {
    ...
    var pluginName = "accessibleMenu",
        defaults = {
            ...
            menuClass: "accessible-menu",
            // default css class used to define the menu styling
            topNavItemClass: "accessible-menu-top-nav-item",
            // default css class for a top-level navigation item in the menu
            panelClass: "accessible-menu-panel",
            // default css class for a menu panel
            panelGroupClass: "accessible-menu-panel-group",
            // default css class for a group of items within a menu panel
            hoverClass: "hover",
    ...
    /**
     * @desc Creates a new accessible menu instance.
     ...
     * @param {string} [options.menuClass=accessible-menu] - CSS class used to define the menu styling
     * @param {string} [options.topNavItemClass=accessible-menu-top-nav-item] - CSS class for a top-level navigation item in the menu
     * @param {string} [options.panelClass=accessible-menu-panel] - CSS class for a menu panel
     * @param {string} [options.panelGroupClass=accessible-menu-panel-group] - CSS class for a group of items within a menu panel
    ...
    AccessibleMenu.prototype = (function () {
         ...
         * @desc Toggle the display of menu panels in response to an event.
         * The optional boolean value 'hide' forces all panels to hide.
         * @param {event} event
         * @param {Boolean} [hide] Hide all menu panels when true
         * @memberof jQuery.fn.accessibleMenu
         * @inner
         * @private
         */
        _togglePanel = function (event, hide) {
            ...
            $('html').off('mouseup.outside-accessible-menu, touchend.outside-accessible-menu, mspointerup.outside-accessible-menu, pointerup.outside-accessible-menu', _clickOutsideHandler);
            ...
            if (hide) {
				...
				if (!(topli.is(event.relatedTarget) || topli.has(event.relatedTarget).length > 0)) {
					...
					if ((event.type === 'mouseout' || event.type === 'focusout') && topli.has(document.activeElement).length > 0) {
						...
						if(menu.find('.' + settings.topNavItemClass).filter('.hover').length === 0 && event.type === 'mouseout'){
                        ...
                        topli
							...
							.find('[tabindex]')
							.attr('tabindex', -1);
                        topli.find('[aria-expanded]').attr('aria-expanded', 'false');
					...
					*/
                    ...
                    if ((event.type === 'keydown' && (event.keyCode === Keyboard.ESCAPE || event.keyCode === Keyboard.LEFT)) || event.type === 'DOMAttrModified') {
                        ...
                        //console.log(target);
						//console.log(target.closest('.' + settings.topNavItemClass));
						
						if (target.closest('.' + settings.topNavItemClass).length === 1 && panel.length === 0){
							newfocus = target;
							menu.find('.' + settings.topNavItemClass)
								...
								.find('[tabindex]')
								.attr('tabindex', -1);
                        menu.find('.' + settings.topNavItemClass).find('[aria-expanded]').attr('aria-expanded', 'false');
				...
				*/
				...
				//console.log(topli.siblings().has('[aria-expanded]'));
         ...
         * @desc Handle click event on menu item
         ...
         * @desc Handle click event outside of a the menu
         * @param {event} Event object
         * @memberof jQuery.fn.accessibleMenu
         * @inner
         * @private
         ...
         * @desc Handle focusin event on menu item.
         * @param {event} Event object
         * @memberof jQuery.fn.accessibleMenu
         * @inner
         * @private
        ...
        _focusInHandler = function (event) {
			...
			//===
            $(event.target).on('click.accessible-menu', _clickHandler.bind(this));
         ...
         * @desc Handle focusout event on menu item.
         * @param {event} Event object
         * @memberof jQuery.fn.accessibleMenu
         * @inner
         * @private
        ...
        _focusOutHandler = function (event) {
			...
			//======
            target.off('click.accessible-menu', _clickHandler);
         ...
         * @desc Handle keydown event on menu.
         * @param {event} Event object
         * @memberof jQuery.fn.accessibleMenu
         * @inner
         * @private
            ...
            switch (keycode) {
                ...
                case Keyboard.UP:
					...
					*/
					...
					if(isTopNavItem){
						...
						if(settings.typeMenu === 'dropdown'){
							if(topli.children('.' + settings.panelClass).length === 1){
								if(topli.has('.' + settings.openClass))
                ...
                default:
                    ...
                    if (keydownSearchString.length === 0) {
						console.log('Return alphanumeric');
                        return;
                    }
					console.log(keydownSearchString)
         ...
         * @desc Handle mousedown event on menu.
         * @param {event} Event object
         ...
         * @desc Handle mouseover event on menu.
         * @param {event} Event object
         * @memberof jQuery.fn.accessibleMenu
         * @inner
         * @private
        ...
        _mouseOverHandler = function (event) {
            ...
            if ($(event.target).is(':tabbable')) {
                $('html').on('keydown.accessible-menu', _keyDownHandler.bind(event.target));
            }
        };

        /**
         * @name jQuery.fn.accessibleMenu~_mouseOutHandler
         * @desc Handle mouseout event on menu.
         * @param {event} Event object
         * @memberof jQuery.fn.accessibleMenu
         * @inner
         * @private
        ...
        _mouseOutHandler = function (event) {
			...
			console.log($(event.target).closest('.' + that.settings.topNavItemClass));
			//_togglePanel.call(that, event, true);
        ...
        _toggleExpandedEventHandlers = function (hide) {
            ...
            if (hide) {
                ...
                menu.find('[aria-expanded].' + this.settings.panelClass).off('DOMAttrModified.accessible-menu', _DOMAttrModifiedHandler);
            } else {
                $('html').on('mouseup.outside-accessible-menu, touchend.outside-accessible-menu, mspointerup.outside-accessible-menu,  pointerup.outside-accessible-menu', _clickOutsideHandler.bind(this));

                /* Narrator in Windows 8 automatically toggles the aria-expanded property on double tap or click. 
                 To respond to the change to collapse the panel, we must add a listener for a DOMAttrModified event. */
                menu.find('[aria-expanded=true].' + this.settings.panelClass).on('DOMAttrModified.accessible-menu', _DOMAttrModifiedHandler.bind(this));
            }
        };
        ...
        return {
            ...
            init: function () {
                ...
                menu.on("focusin.accessible-menu", ":tabbable, :focusable, ." + settings.panelClass, _focusInHandler.bind(this))
                    .on("focusout.accessible-menu", ":tabbable, :focusable, ." + settings.panelClass, _focusOutHandler.bind(this))
                    .on("keydown.accessible-menu", _keyDownHandler.bind(this))
                    .on("mouseover.accessible-menu", _mouseOverHandler.bind(this))
                    .on("mouseout.accessible-menu", _mouseOutHandler.bind(this))
                    .on("mousedown.accessible-menu", _mouseDownHandler.bind(this));

                if (isTouch) {
                    menu.on("touchstart.accessible-menu", _clickHandler.bind(this));
                }

                menu.find("hr").attr("role", "separator");
                
                menu.find("a").filter(function() { return this.href == window.location.href; })
                    .closest('li')
                    .addClass('active');
            },
